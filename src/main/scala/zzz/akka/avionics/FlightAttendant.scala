...
class FlightAttendant extends Actor {
  ...
  def handleDrinkRequest: Receive = {
  ...
  def handleSpecificPerson(person: ActorRef): Receive = {
    ...
    case DeliverDrink(drink) =>
      person ! drink
      pendingDelivery = None
      context.become(assistInjuredPassenger orElse handleDrinkRequest)
    case m: GetDrink =>
  ...
  def receive = assistInjuredPassenger orElse handleDrinkRequest
}